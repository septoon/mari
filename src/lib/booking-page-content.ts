...
const BOOKING_PAGE_DEFAULTS: BookingPageContent = {
  heroActions: {
...
export const resolveBookingPageContent = (
  extra: Record<string, unknown> | null | undefined
): BookingPageContent => {
  const source = asObjectRecord(asObjectRecord(extra).bookingPage);

  return {
    heroActions: mergeStringSection(BOOKING_PAGE_DEFAULTS.heroActions, source.heroActions),
    connectivityNotice: mergeStringSection(
      BOOKING_PAGE_DEFAULTS.connectivityNotice,
      source.connectivityNotice
    ),
    panel: mergeStringSection(BOOKING_PAGE_DEFAULTS.panel, source.panel),
    schedule: mergeStringSection(BOOKING_PAGE_DEFAULTS.schedule, source.schedule),
    confirmation: mergeStringSection(BOOKING_PAGE_DEFAULTS.confirmation, source.confirmation)
  };
};

export const bookingPageDefaults = BOOKING_PAGE_DEFAULTS;
