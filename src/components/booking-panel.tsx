...
export function BookingPanel({
  ...
  const availableSpecialists = specialists.filter((specialist) =>
    selectedServiceIds.length === 0
      ? true
      : selectedServiceIds.every((serviceId) => specialist.services.some((service) => service.id === serviceId))
  );
  const totalMin = selectedServices.reduce((sum, service) => sum + service.priceMin, 0);
  const totalMax = selectedServices.reduce((sum, service) => sum + (service.priceMax ?? service.priceMin), 0);
  ...
  const hasPermanentDiscount = Boolean(session.authenticated && permanentDiscountPercent && permanentDiscountPercent > 0);
  const discountedMin = hasPermanentDiscount ? applyPercentDiscount(totalMin, permanentDiscountPercent!) : totalMin;
  const discountedMax = hasPermanentDiscount ? applyPercentDiscount(totalMax, permanentDiscountPercent!) : totalMax;
  ...
  useEffect(() => {
    if (selectedStaffId !== 'any' && !availableSpecialists.some((item) => item.staffId === selectedStaffId)) {
  ...
  useEffect(() => {
    const handleServicePicked = (event: Event) => {
      ...
      setSelectedServiceIds((current) =>
        current.includes(detail.serviceId as string) ? current : [...current, detail.serviceId as string]
      );
      setCatalogOpen(true);
      setSuccess(null);
      setErrorMessage(null);
  ...
  return (
    <section
      id="booking"
      className="grid gap-6 rounded-[2rem] border border-[color:var(--line)] bg-[linear-gradient(135deg,rgba(11,62,69,0.96),rgba(16,83,91,0.92))] p-5 text-white shadow-[0_35px_100px_rgba(7,33,38,0.28)] md:p-6 lg:grid-cols-[1.08fr_0.92fr]"
    >
      <div className="space-y-5">
        ...
        <div className="rounded-[1.55rem] border border-white/14 bg-white/8 p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-white/72">
                Добавляйте услуги прямо отсюда. Выбранные позиции сразу влияют на время визита, стоимость и доступные окна.
              </p>
            </div>
            ...
            <button
              ...
              className="rounded-full border border-white/14 bg-white/10 px-4 py-2 text-sm text-white/82 transition hover:bg-white/14"
            >
              {catalogOpen ? 'Скрыть каталог' : 'Добавить услуги'}
            </button>
          </div>

          <div className="mt-5 grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
            <div className="rounded-[1.35rem] border border-white/10 bg-[#f7f3ed] p-4 text-[color:var(--ink)]">
              ...
              {selectedServices.length ? (
                <div className="mt-4 space-y-3">
                  {selectedServices.map((service) => (
                    <div
                      key={service.id}
                      className="flex items-start justify-between gap-3 rounded-[1.2rem] border border-[color:var(--line)] bg-white px-4 py-3"
                    >
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs text-[color:var(--ink-muted)]">
                          {formatDuration(service.durationSec)} · {formatPriceRange(service.priceMin, service.priceMax)}
                        </p>
                      </div>
                      <button
                        type="button"
            ...
            <div className="rounded-[1.35rem] border border-white/10 bg-white/6 p-4">
              ...
              <div className="mt-4 space-y-3 text-sm text-white/82">
                <div className="flex items-center justify-between gap-4">
                  <span>Услуги</span>
                  <span>{selectedServices.length || 0}</span>
                </div>
                <div className="flex items-center justify-between gap-4">
                  <span>Длительность</span>
                  <span>{selectedServices.length ? formatDuration(totalDurationSec) : 'Пока не выбрано'}</span>
                </div>
                <div className="flex items-center justify-between gap-4">
                  <span>Бюджет</span>
                  <span>{selectedServices.length ? formatPriceRange(totalMin, totalMax) : 'Пока не выбрано'}</span>
                </div>
                {hasPermanentDiscount ? (
                  <div className="rounded-[1.1rem] border border-[#f0d78b]/35 bg-[#fff7dd] px-4 py-3 text-sm text-[#73571d]">
                    Постоянная скидка: <span className="font-semibold">-{permanentDiscountPercent}%</span>
                  </div>
                ) : null}
              </div>
            </div>
            ...
            <div className="mt-5 rounded-[1.35rem] border border-white/10 bg-white/6 p-4">
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <span className={`mt-2 block text-xs ${active ? 'text-[color:var(--ink-muted)]' : 'text-white/58'}`}>
                              {formatDuration(service.durationSec)} · {formatPriceRange(service.priceMin, service.priceMax)}
                            </span>
                          </div>
                          <span
              ...
              {filteredServices.length > visibleServices.length ? (
                <p className="mt-3 text-xs text-white/55">
                  Показаны первые {visibleServices.length} услуг из {filteredServices.length}. Уточните поиск или категорию.
                </p>
              ) : null}
            </div>
          ) : null}
        ...
        <div className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
          <div className="rounded-[1.5rem] border border-white/14 bg-white/8 p-5">
            <p className="text-xs uppercase tracking-[0.28em] text-white/58">Когда и к кому</p>
            <div className="mt-4 grid gap-4 md:grid-cols-[1fr_1fr_auto]">
              <label className="space-y-2">
                <span className="text-xs uppercase tracking-[0.24em] text-white/58">Дата</span>
                <input
                  type="date"
                  min={getLocalDate(0)}
                  value={selectedDate}
                  onChange={(event) => setSelectedDate(event.target.value)}
                  className="w-full rounded-full border border-white/16 bg-white/8 px-4 py-3 text-sm outline-none transition focus:border-white/30"
                />
              </label>
              ...
              <div className="rounded-[1.2rem] border border-white/12 bg-white/8 px-4 py-3 text-sm text-white/80">
          ...
          <div className="rounded-[1.5rem] border border-white/14 bg-white/8 p-5">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.28em] text-white/58">Свободные слоты</p>
                <p className="mt-2 text-sm text-white/70">После выбора корзины и даты показываем только подходящие окна.</p>
              </div>
              {loadingSlots ? <span className="text-sm text-white/60">Обновляю...</span> : null}
            </div>

            <div className="mt-5 space-y-4">
              {!selectedServiceIds.length ? (
                <p className="text-sm text-white/70">Сначала добавьте услуги в корзину.</p>
              ) : slots && slots.results.some((group) => group.slots.length > 0) ? (
                slots.results.map((group) => (
                  <div key={group.staffId} className="space-y-3">
                    <p className="text-xs uppercase tracking-[0.24em] text-white/56">{group.staffName}</p>
                    <div className="flex flex-wrap gap-2">
                      {group.slots.length === 0 ? (
                        <span className="rounded-full border border-white/12 px-4 py-2 text-sm text-white/44">
                          Нет окон
                        </span>
                      ) : (
                        group.slots.map((slot) => {
                          const active =
                            selectedSlot?.staffId === group.staffId && selectedSlot?.startAt === slot.startAt;
                ...
                <p className="text-sm text-white/70">На выбранную дату свободных окон пока нет.</p>
              )}
            </div>
          </div>
        </div>
      ...
      <form
        ...
        className="space-y-5 rounded-[1.75rem] bg-white p-6 text-[color:var(--ink)] shadow-[0_25px_70px_rgba(7,33,38,0.22)] lg:sticky lg:top-24 lg:self-start"
      >
              <div>
                <p className="text-xs uppercase tracking-[0.28em] text-[color:var(--ink-muted)]">Подтверждение</p>
        ...
        {!session.authenticated ? (
          ...
          <div className="rounded-[1.3rem] border border-[color:var(--line)] bg-[color:var(--panel)] px-4 py-4 text-sm text-[color:var(--ink-muted)]">
            ...
            <p className="mt-2">{session.client?.name || 'Имя пока не заполнено'} · {session.client?.phoneE164}</p>
            {hasPermanentDiscount ? (
              <div className="mt-3 rounded-2xl border border-[#f0d78b] bg-[#fff8df] px-4 py-3 text-sm text-[#7d6120]">
                Ваша постоянная скидка: <span className="font-semibold">{permanentDiscountPercent}%</span>.
                Она применяется автоматически, пока вы не используете промокод.
              </div>
            ) : null}
          </div>
        )}
        ...
        {session.authenticated ? (
          <div className="grid gap-4 md:grid-cols-2">
            <label className="space-y-2">
              <span className="text-xs uppercase tracking-[0.24em] text-[color:var(--ink-muted)]">Промокод</span>
              <input
                value={promoCode}
            ...
            <label className="space-y-2">
              <span className="text-xs uppercase tracking-[0.24em] text-[color:var(--ink-muted)]">Комментарий</span>
              <input
                value={comment}
        ...
        <div className="rounded-[1.35rem] border border-[color:var(--line)] bg-[linear-gradient(180deg,rgba(244,240,235,0.94),rgba(235,241,239,0.8))] px-5 py-4">
          ...
          <div className="mt-3 space-y-2 text-sm text-[color:var(--ink-muted)]">
            ...
            {hasPermanentDiscount ? (
              <p>
                Постоянная скидка: <span className="font-semibold text-[color:var(--ink)]">-{permanentDiscountPercent}%</span>
              </p>
            ) : null}
            ...
            {hasPermanentDiscount && !promoCode.trim() ? (
              <p className="text-xs text-[color:var(--ink-muted)]">
                Базовая стоимость без скидки: {formatPriceRange(totalMin, totalMax)}
              </p>
            ) : null}
            {hasPermanentDiscount && promoCode.trim() ? (
              <p className="text-xs text-[color:var(--ink-muted)]">
                Если промокод сработает, он заменит постоянную скидку.
              </p>
            ) : null}
          </div>
        </div>
        ...
        {success ? (
          <div className="rounded-[1.2rem] border border-[#86aa96]/40 bg-[#e3f0e8] px-4 py-4 text-sm text-[#21573b]">
            ...
            <p className="mt-1">
              {success.staff.name} · {formatBookingDateTime(success.startAt)} · {formatCurrency(success.prices.finalTotal)}
            </p>
          </div>
        ) : null}
        ...
        {session.authenticated ? (
          ...
          >
            {isSubmitting ? 'Создаю запись...' : 'Подтвердить запись'}
          </button>
        ) : (
