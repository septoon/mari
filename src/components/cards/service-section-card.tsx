...
export function ServiceSectionCard({
  ...
  const cardLabels = {
    eyebrow: labels?.eyebrow ?? 'Раздел услуг',
    fallbackText: labels?.fallbackText ?? 'Место под фото раздела',
    serviceCountLabel: labels?.serviceCountLabel ?? `${section.servicesCount} услуг`,
    actionLabel: labels?.actionLabel ?? 'Открыть',
  };
