  ...
  <div className="grid gap-4 lg:grid-cols-2">
    {items.map((contact) => {
      ...
      return (
        <article
          key={contact.id}
          className="rounded-[1.5rem] border border-[color:var(--line)] bg-white/78 p-5 shadow-[0_20px_45px_rgba(12,77,85,0.06)]"
        >
          <p className="text-xs uppercase tracking-[0.24em] text-[color:var(--ink-muted)]">
            {contact.publicName || contact.name}
          </p>
          <h3 className="mt-3 font-serif text-3xl text-[color:var(--ink)]">{primaryAddress?.label || 'Контакт'}</h3>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            {primaryPhone ? (
                <a
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-[color:var(--button-bg)] px-4 py-2 text-sm font-medium text-white"
                >
            ...
            {contact.mapUrl ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 rounded-full border border-[color:var(--line)] px-4 py-2 text-sm text-[color:var(--ink)]"
              >
                Открыть карту
  ...
  return (
    <div className="space-y-8">
      {promoBlocks.length > 0 ? (
        <section className="grid gap-5 lg:grid-cols-2">
          {promoBlocks.map((block) => (
            <article
              key={block.id}
              className="rounded-[2rem] border border-[color:var(--line)] bg-[linear-gradient(135deg,rgba(233,239,237,0.88),rgba(255,255,255,0.95))] p-6 shadow-[0_25px_70px_rgba(12,77,85,0.06)]"
            >
              <p className="inline-flex items-center gap-2 rounded-full border border-[color:var(--line)] px-4 py-2 text-xs uppercase tracking-[0.28em] text-[color:var(--ink-muted)]">
                <Sparkles className="h-4 w-4" />
                {block.payload.badge || 'Спецпредложение'}
              </p>
              <h2 className="mt-5 font-serif text-4xl text-[color:var(--ink)]">{block.payload.title}</h2>
              <p className="mt-3 max-w-xl text-sm leading-7 text-[color:var(--ink-muted)]">
              ...
              {block.payload.ctaUrl && block.payload.ctaText ? (
                <a
                  ...
                  className="mt-6 inline-flex items-center gap-2 rounded-full bg-[color:var(--button-bg)] px-5 py-3 text-sm font-medium text-white"
                >
      ...
      {offerBlocks.length > 0 ? (
        <section className="rounded-[2rem] border border-[color:var(--line)] bg-white/74 px-6 py-8 shadow-[0_25px_70px_rgba(12,77,85,0.06)]">
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-3">
            {offerBlocks.flatMap((block) => block.payload.items).map((offer) => (
              <article key={offer.id} className="rounded-[1.5rem] border border-[color:var(--line)] bg-[color:var(--panel)] p-5">
                <p className="text-xs uppercase tracking-[0.24em] text-[color:var(--ink-muted)]">
                  {offer.subtitle || 'Offer'}
                </p>
                <h3 className="mt-3 font-serif text-3xl text-[color:var(--ink)]">{offer.title}</h3>
                <p className="mt-3 text-sm leading-6 text-[color:var(--ink-muted)]">
                  {offer.description || 'Продуманный формат визита для тех, кто хочет больше заботы за одно посещение.'}
                </p>
                <div className="mt-5 flex items-baseline gap-3">
                  {offer.finalPrice ? (
                    <p className="text-2xl font-semibold text-[color:var(--ink)]">{formatCurrency(offer.finalPrice)}</p>
                  ) : null}
                  {offer.originalPrice ? (
                    <p className="text-sm text-[color:var(--ink-muted)] line-through">{formatCurrency(offer.originalPrice)}</p>
                  ) : null}
                </div>
              </article>
            ))}
      ...
      {textBlocks.length > 0 ? (
        <section className="grid gap-5 lg:grid-cols-2">
          {textBlocks.map((block) => (
            <article
              key={block.id}
              className="rounded-[1.7rem] border border-[color:var(--line)] bg-[color:var(--panel-strong)] p-6"
            >
              {block.payload.title ? <p className="section-kicker">{block.payload.title}</p> : null}
              <div className="mt-3 whitespace-pre-line text-sm leading-7 text-[color:var(--ink-muted)]">{block.payload.body}</div>
            </article>
          ))}
        </section>
      ) : null}

      <section className="rounded-[2rem] border border-[color:var(--line)] bg-white/78 px-6 py-8 shadow-[0_25px_70px_rgba(12,77,85,0.06)]">
        <div className="max-w-2xl">
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-3">
          {(faqItems.length > 0 ? faqItems : fallbackFaq).map((item) => (
            <article key={item.question} className="rounded-[1.5rem] border border-[color:var(--line)] bg-[color:var(--panel)] p-5">
              <p className="inline-flex items-center gap-2 text-sm font-medium text-[color:var(--ink)]">
                <MessageCircleMore className="h-4 w-4" />
                {item.question}
              </p>
              <p className="mt-3 text-sm leading-6 text-[color:var(--ink-muted)]">{item.answer}</p>
            </article>
          ))}
        </div>
      </section>
