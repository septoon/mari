...
export function EditorialVisual({
  ...
  return (
    <div className={cn('relative isolate overflow-hidden rounded-[2rem] border border-white/70 bg-[linear-gradient(145deg,rgba(255,255,255,0.95),rgba(247,241,234,0.88)_52%,rgba(234,227,219,0.8)_100%)] p-6 shadow-[0_30px_90px_rgba(69,48,29,0.08)] md:p-8', className)}>
      ...
      <div className="absolute right-8 top-8 text-[color:var(--accent-strong)]">
      ...
      <div className="relative flex min-h-[20rem] flex-col justify-between">
        <div className="flex justify-between gap-4">
          <Badge>{label}</Badge>
          <div className="hidden h-16 w-16 items-center justify-center rounded-full border border-[color:var(--line)] bg-white/88 font-serif text-4xl text-[color:var(--ink)] md:flex">
            {monogram}
          </div>
        </div>

        <div className="space-y-4">
          <h3 className="max-w-md font-serif text-4xl leading-[1.02] text-[color:var(--ink)] md:text-5xl">
            {title}
          </h3>
          <p className="max-w-md text-sm leading-7 text-[color:var(--muted)]">{subtitle}</p>
        </div>
      </div>
    </div>
  );
